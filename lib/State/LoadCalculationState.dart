...
class LoadCalculationState extends ChangeNotifier{
  ...
  void incrementComponentQuantity(int index){
    ...
    for (int i=0; i<_components.length;i++) {
      _runningPower += _components[i].runningPower!.toDouble() * _componentQuantity[i].toDouble();
    }
    for (int i=0; i<_components.length;i++) {
      _startingPower += _components[i].startPower!.toDouble() * _componentQuantity[i].toDouble();
    }
    CalculateKVA();
    notifyListeners();
  }
  ...
  Equipment getcomponentByName(String Name) => _components.where((element) => element.name == Name).first;
  ...
  void incrementFieldsCount(){
    _fieldsCount++;
    _components.add(_allComponents.elementAt(Random().nextInt(_allComponents.length)));
